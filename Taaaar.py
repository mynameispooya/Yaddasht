...
if uploaded_files:
    if st.button("شروع محاسبه 🚀"):
        ...
        # استفاده از applymap برای سازگاری با نسخه‌های مختلف پانداز (در نسخه‌های جدیدتر معادل map است)
        apply_func = getattr(pd.io.formats.style.Styler, 'map', getattr(pd.io.formats.style.Styler, 'applymap', None))
        
        for fname, idf in all_individual_dfs.items():
            with st.expander(f"📁 بررسی ریز گزارش فایل: {fname}", expanded=False):
                if apply_func:
                    styled_df = idf.style
                    styled_df = apply_func(styled_df, lambda x: highlight_cells(x, '#d4edda'), subset=['کارکرد روزانه'])
                    styled_df = apply_func(styled_df, lambda x: highlight_cells(x, '#cce5ff'), subset=['اضافه کاری'])
                    styled_df = apply_func(styled_df, lambda x: highlight_cells(x, '#ffeeba'), subset=['اضافه کاری روز تعطیل'])
                    styled_df = apply_func(styled_df, lambda x: highlight_cells(x, '#e2d9f3'), subset=['جمعه کاری'])
                    styled_df = apply_func(styled_df, highlight_day_type, subset=['نوع روز'])
                    styled_df = apply_func(styled_df, lambda x: 'background-color: #d1ecf1; font-weight: bold' if x == 'مجموع' else '', subset=['روز'])
