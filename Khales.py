...
def parse_attendance_file(file):
    ...
    # 1. پیدا کردن سطر هدر اصلی (سطری که شامل کلمه 'تاریخ' باشد)
    header_idx = -1
    ...
    if header_idx == -1:
        return pd.read_excel(file) # بازگشت به حالت استاندارد اگر هدر پیچیده نبود
    ...
    # 2. ترکیب هدرها و حذف ستون 'اطلاعات تردد'
    final_columns = []
    for i in range(len(main_headers)):
        ...
        if mh in ['اطلاعات تردد', 'nan', '']:
            final_columns.append(sh if sh and sh != 'nan' else (mh if mh and mh != 'nan' else f'Col_{i}'))
        else:
            final_columns.append(mh)
            
    # تعیین نقطه شروع داده‌ها
    ...
    for col in df_clean.columns:
        if df_clean[col].dtype == object:
            # حذف کاراکتر \u200f که باعث خطای محاسباتی می‌شود
...
def time_to_minutes(t_str):
    ...
    try:
        ...
        # تبدیل اعداد فارسی به انگلیسی (برای احتیاط)
...
# ==========================================
# 3. تجمیع و عارضه‌یابی پیشرفته (Anchor + System Sync)
# ==========================================
...
def sync_and_diagnose(daily_df, shift_pattern):
    ...
    for i, row in daily_df.iterrows():
        ...
        # 1. کشف عدم ثبت ورود/خروج (سناریوی ۱۴۰۵/۰۶/۰۹)
        ...
        # 2. کشف خروج از الگو (سناریوی ۱۴۰۵/۰۶/۱۳)
        if actual_in != '-':
