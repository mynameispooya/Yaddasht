        ...
        for i, file in enumerate(uploaded_files):
            ...
            for index, row in df.iterrows():
                ...
                if date_str not in daily_stats:
                    ...
                    is_manual_holiday = date_str in manual_holidays_list
        ...
        for fname, idf in all_individual_dfs.items():
            with st.expander(f"📁 بررسی ریز گزارش فایل: {fname}", expanded=False):
                ...
                if hasattr(styled_df, 'map'):
                    # برای Pandas >= 2.1.0
                    styled_df = styled_df.map(lambda x: highlight_cells(x, '#d4edda'), subset=['کارکرد روزانه'])
                    styled_df = styled_df.map(lambda x: highlight_cells(x, '#cce5ff'), subset=['اضافه کاری'])
                    styled_df = styled_df.map(lambda x: highlight_cells(x, '#ffeeba'), subset=['اضافه کاری روز تعطیل'])
                    styled_df = styled_df.map(lambda x: highlight_cells(x, '#e2d9f3'), subset=['جمعه کاری'])
                    styled_df = styled_df.map(highlight_day_type, subset=['نوع روز'])
                    styled_df = styled_df.map(lambda x: 'background-color: #d1ecf1; font-weight: bold' if x == 'مجموع' else '', subset=['روز'])
                else:
                    # برای Pandas < 2.1.0
                    styled_df = styled_df.applymap(lambda x: highlight_cells(x, '#d4edda'), subset=['کارکرد روزانه'])
                    styled_df = styled_df.applymap(lambda x: highlight_cells(x, '#cce5ff'), subset=['اضافه کاری'])
                    styled_df = styled_df.applymap(lambda x: highlight_cells(x, '#ffeeba'), subset=['اضافه کاری روز تعطیل'])
                    styled_df = styled_df.applymap(lambda x: highlight_cells(x, '#e2d9f3'), subset=['جمعه کاری'])
                    styled_df = styled_df.applymap(highlight_day_type, subset=['نوع روز'])
                    styled_df = styled_df.applymap(lambda x: 'background-color: #d1ecf1; font-weight: bold' if x == 'مجموع' else '', subset=['روز'])
