...
st.info("نکته: شروع کار 07:15 است. پایان کار در روزهای عادی 16:00 و در روزهای پنج‌شنبه 12:45 می‌باشد. خروج پس از این ساعات به عنوان اضافه‌کاری محاسبه می‌گردد.")
...
if uploaded_files:
    if st.button("شروع محاسبه 🚀"):
        ...
        for i, file in enumerate(uploaded_files):
            ...
            for index, row in df.iterrows():
                ...
                if vazeiat == 'حضور' and not pd.isna(start_t) and not pd.isna(end_t):
                    ...
                    is_thursday = 'پنج' in day_name # تشخیص روز پنج‌شنبه
                    ...
                    if is_friday:
                        ...
                        # سقف ساعت کاری: پنج‌شنبه 12:45 (765 دقیقه) و سایر روزها 16:00 (960 دقیقه)
                        regular_end_limit = 765 if is_thursday else 960
                        
                        calc_end_reg = min(t_end, regular_end_limit) 
                        if calc_end_reg > t_start:
                            daily_stats[date_str]['کارکرد روزانه'] += (calc_end_reg - t_start)
                            
                        calc_start_ot = max(t_start, regular_end_limit) 
        ...
        # ---------------- نمایش ریز گزارش هر شخص با هایلایت و امکان دانلود مجزا ----------------
        st.markdown("---")
        st.subheader("📋 ریز محاسبات اشخاص (جهت بررسی، تایید و دانلود)")
        ...
        for fname, idf in all_individual_dfs.items():
            with st.expander(f"📁 بررسی ریز گزارش فایل: {fname}", expanded=False):
                ...
                # ایجاد فایل اکسل اختصاصی برای این شخص در حافظه موقت
                indiv_buffer = io.BytesIO()
                with pd.ExcelWriter(indiv_buffer, engine='openpyxl') as writer:
                    idf.to_excel(writer, sheet_name='ریز_محاسبات', index=False)
                
                # دکمه دانلود اختصاصی با یک key یکتا
                st.download_button(
                    label=f"📥 دانلود اکسل ریز محاسبات مربوط به {fname}",
                    data=indiv_buffer.getvalue(),
                    file_name=f"ریز_گزارش_{fname}",
        ...
        st.download_button(
            label="📥 دانلود فایل اکسل گزارش نهایی (شامل همه افراد)",
